...
export default function LayoutPage() {
        ...
        prev.map((f) =>
          f.floorId !== activeFloorId
            ...
            : {
                ...f,
                areas: f.areas.map((a) =>
                  a.areaId !== activeAreaId ? a : { ...a, tables: [...a.tables, newTableLocal] }
                )
              }
        )
      )
      ...
      prev.map((f) =>
        f.floorId !== activeFloorId
          ...
          : {
              ...
              areas: f.areas.map((a) => ({
                ...a,
                tables: a.tables.map((t) => (t.id === selectedTableId ? { ...t, ...patch } : t))
              }))
            }
      )
    )
  ...
  return (
    <div className='flex min-h-screen flex-col bg-white'>
      <header className='border-b px-6 py-4 font-bold'>CrownDine – Quản lý sơ đồ bàn</header>

      <div className='flex flex-1 overflow-hidden'>
        {/* ================= LEFT SIDEBAR ================= */}
        <div className='w-64 overflow-y-auto border-r bg-white p-4'>
          <Button className='mb-4 w-full gap-2 bg-orange-500 hover:bg-orange-600' onClick={() => setShowAddArea(true)}>
            <Plus size={16} /> Thêm khu vực
          </Button>

          <Button
            className='mb-6 w-full gap-2 bg-blue-600 text-white hover:bg-blue-700'
          ...
          {floors.map((floor) => (
            <div key={floor.floorId} className='mb-2'>
              <button
                ...
                onClick={() => {
                  setExpandedFloors((p) =>
                    p.includes(floor.floorId) ? p.filter((i) => i !== floor.floorId) : [...p, floor.floorId]
                  )
              ...
              >
                {expandedFloors.includes(floor.floorId) ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                {floor.floorName}
              </button>
              ...
              {expandedFloors.includes(floor.floorId) &&
                floor.areas.map((area) => (
                  <button
                    ...
                    className={`mt-1 w-full rounded-md border-l-4 px-3 py-2 text-left transition ${
        ...
        <div className='flex flex-1 flex-col bg-gray-50 p-6'>
          {!activeFloor ? (
            ...
            /* ================= AREA EDIT MODE ================= */
            <>
              <h2 className='mb-4 text-xl font-bold'>Sơ đồ khu vực - {activeFloor.floorName}</h2>
              <div className='mb-4 flex gap-2'>
                <Button variant='outline' onClick={() => setShowAddArea(true)}>
                  + Thêm khu vực mới
                </Button>
                <div className='ml-auto flex gap-2'>
                  <div className='rounded-md border border-green-200 bg-green-100 px-3 py-1 text-sm font-semibold text-green-700'>
              ...
              >
                <AreaCanvas
                  ...
                  onChange={(updated) =>
                    setFloors((prev) => prev.map((f) => (f.floorId !== activeFloor.floorId ? f : updated)))
                  }
                />
              </div>
            </>
        ...
        <div className='w-80 overflow-y-auto border-l bg-white p-6'>
          {activeAreaId && selectedTable ? (
              ...
              <div className='space-y-4'>
                ...
                <Field label='Số khách tối đa'>
                  <Input
                    ...
                    onChange={(e) => updateTableState({ capacity: Number(e.target.value) })}
                  />
                </Field>
                ...
                <Field label='Tiền cọc (VND)'>
                  <Input
                    ...
                    onChange={(e) => updateTableState({ deposit: Number(e.target.value) })}
                  />
                </Field>
                ...
                <div className='grid grid-cols-2 gap-2'>
                  <Field label='Chiều rộng (px)'>
                    <Input
                      ...
                      onChange={(e) => updateTableState({ width: Number(e.target.value) })}
                    />
                  </Field>
                  <Field label='Chiều cao (px)'>
                    <Input
                      ...
                      onChange={(e) => updateTableState({ height: Number(e.target.value) })}
                    />
                  </Field>
                </div>
                ...
                <Field label='Góc xoay (°)'>
                  <Input
                    ...
                    onChange={(e) => updateTableState({ rotation: Number(e.target.value) })}
                  />
                </Field>
                ...
                <Field label='Hình dạng'>
                  <select
                    ...
                    onChange={(e) => updateShape(e.target.value as TableLayout['shape'])}
                  >
                ...
                <Button className='mt-4 w-full' onClick={handleUpdateTableApi}>
                  Cập nhật Bàn
                </Button>
                <Button variant='destructive' className='mt-2 w-full' onClick={handleDeleteTableApi}>
                  Xóa Bàn
                </Button>
              </div>
            </>
                    ...
                    <div className='grid grid-cols-2 gap-2'>
                      <Field label='Vị trí X (px)'>
                        <Input
                          ...
                          onChange={(e) => updateAreaState({ x: Number(e.target.value) })}
                        />
                      </Field>
                      <Field label='Vị trí Y (px)'>
                        <Input
                          ...
                          onChange={(e) => updateAreaState({ y: Number(e.target.value) })}
                        />
                      </Field>
                    </div>
                    ...
                    <div className='grid grid-cols-2 gap-2'>
                      <Field label='Chiều rộng (px)'>
                        <Input
                          ...
                          onChange={(e) => updateAreaState({ width: Number(e.target.value) })}
                        />
                      </Field>
                      <Field label='Chiều cao (px)'>
                        <Input
                          ...
                          onChange={(e) => updateAreaState({ height: Number(e.target.value) })}
                        />
                      </Field>
                    </div>
                  </div>
            ...
            <div className='mt-20 text-center text-gray-400'>Chọn một Mục trên sơ đồ để xem chi tiết</div>
          )}
        </div>
      </div>

      {/* Confirm Update Modal */}
      <Modal isOpen={isUpdateModalOpen} onClose={() => setIsUpdateModalOpen(false)} title='Xác nhận'>
        <div className='flex flex-col gap-4'>
          ...
          <div className='mt-4 flex justify-end gap-3'>
            ...
            <button className='rounded bg-blue-600 px-4 py-2 text-white hover:bg-blue-700' onClick={confirmUpdateTable}>
              Cập nhật
            </button>
          </div>
        </div>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title='Xác nhận xóa'>
        <div className='flex flex-col gap-4'>
          ...
          <div className='mt-4 flex justify-end gap-3'>
            ...
            <button className='rounded bg-red-500 px-4 py-2 text-white hover:bg-red-600' onClick={confirmDeleteTable}>
              Xóa bàn
            </button>
          </div>
        </div>
      ...
      <Modal isOpen={isSaveModalOpen} onClose={() => setIsSaveModalOpen(false)} title='Xác nhận lưu sơ đồ'>
        <div className='flex flex-col gap-4'>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40'>
          <div className='w-[420px] space-y-4 rounded-lg bg-white p-6'>
            <div className='flex items-center justify-between'>
              <h3 className='text-lg font-bold'>Thêm Khu vực Mới</h3>
              <X className='cursor-pointer' onClick={() => setShowAddArea(false)} />
            </div>
            ...
            <div className='flex justify-end gap-2 pt-4'>
              ...
              <Button onClick={handleSaveArea}>Lưu khu vực</Button>
            </div>
          </div>
        </div>
      )}
