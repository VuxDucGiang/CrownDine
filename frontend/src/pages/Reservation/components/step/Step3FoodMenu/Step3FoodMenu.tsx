    ...
    <div className='animate-fade-in flex flex-col overflow-hidden lg:h-[700px] lg:flex-row'>
      ...
      <div className='bg-muted/10 flex flex-1 flex-col overflow-hidden border-r p-4 lg:p-6'>
        <div className='mb-4 flex items-center justify-between'>
          <div>
            <h3 className='dark:text-white-900 text-lg font-bold'>Chọn món ăn & combo</h3>
            <p className='text-muted-foreground text-xs'>Sẵn sàng phục vụ khi bạn tới nhà hàng</p>
          </div>
          {expiratedAt && (
            <div className='flex items-center gap-2 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-600'>
              <CountdownTimer
      ...
      <div className='bg-background flex w-full flex-col border-t lg:w-[400px] lg:border-t-0'>
            ...
            <div className='space-y-4'>
              {cartItems.map((item) => (
                <div
                  ...
                  className='group border-border flex flex-col gap-3 border-b pb-4 last:border-0 last:pb-0'
                >
                  <div className='flex gap-3'>
                    ...
                    <div className='flex flex-1 flex-col justify-between py-0.5'>
                      <div className='flex items-start justify-between gap-2'>
                        <h5 className='line-clamp-1 text-sm font-bold'>{item.name}</h5>
                        <button
                      ...
                      <div className='mt-2 flex items-center justify-between'>
                        ...
                        <div className='border-border flex items-center gap-3 rounded-md border bg-white px-1 py-0.5'>
                          ...
                          >
                            <Minus size={12} className='dark:text-black' />
                          </button>
                          <span className='w-4 text-center text-xs font-bold dark:text-black'>{item.quantity}</span>
                          <button
                            onClick={() => updateQuantity(item.type, item.id, 1)}
                          ...
                          >
                            <Plus size={12} className='dark:text-black' />
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className='relative mt-1'>
                    ...
                    <input
                      ...
                      className='bg-muted/30 border-border focus:border-primary w-full rounded-md border py-1.5 pr-3 pl-8 text-[11px] transition-all outline-none focus:bg-white'
                    />
                  </div>
                </div>
              ))}
        ...
        <div className='bg-muted/30 border-border border-t p-4 lg:p-6'>
          ...
          <div className='border-white-300 mb-4 flex items-center justify-between border-t border-dashed pt-4'>
            <span className='text-sm font-bold tracking-tight uppercase'>Tổng cộng</span>
            <span className='text-primary text-xl font-black'>{formatCurrency(foodTotal)}</span>
          </div>

          <div className='flex items-center gap-2 rounded-lg border border-blue-100 bg-blue-50 p-2'>
